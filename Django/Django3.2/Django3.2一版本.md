...
@require_http_methods(["POST"])   # 记住这个POST是大写的
def login(request):
...
```python 
print(request.method)   # 输入posthuo'zhe
```
...
<script>
    ...
    $(":text").blur(function () {
        // 发送Ajax请求
        $.ajax({
            url: "http://127.0.0.1:8008/reg_user/",  #请求的地址
            type: "post",     
            data: {
                user: $(":text").val(),   # data里面是前端传到后端的数据
            },
            success: function (response) {  # success是后端把数据处理后在给前端的返回  ，response就是后端返回给前端的数据。
...
1.csrf_token会渲染一个input标签

2.点击提交后，会在发给服务器，进行校验。
...
```js
<input type="text" onblur="foo()" id="l1" class="l1">
...
type  是访问的方式
data  是前端给后端的数据
success: function (response){}    response 是后端处理好的数据，返回给了前端，在前端在做处理
```



如果后端用的是JsonResponse，前端不用反序列化了
